...
const FileCard = ({file, onDelete, onTogglePublic, onDownload, onShareLink}) => {
    ...
    const getFileIcon = (file) => {
        ...
        if (['jpg', 'jpeg', 'png', 'gif', 'svg', 'webp'].includes(extenstion)) {
            return <Image size={24} className="text-sky-700" />
        }
        ...
        return <FileIcon size={24} className="text-sky-700" />
    }
                ...
                <div className="flex gap-3 w-full justify-center">
                    {file.isPublic && (
                        <button
                            ...
                            title="Share Link"
                            className="p-2 cursor-pointer bg-white/90 rounded-full hover:bg-white transition-colors text-sky-700 hover:text-sky-900">
                            <Copy size={18} />
                        </button>
                    )}
